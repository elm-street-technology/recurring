...
class RecurlyData {
  constructor(options) {
    ...
    this.__defineGetter__('href', function() {
      ...
      this._href = [this.constructor.ENDPOINT, this.id].join('/')
      return this._href
    })

    this.__defineSetter__('href', href => {
      this._href = href
    })
  }
  ...
  inflate(json) {
    ...
    // Setting the ID mutates other properties, so do that first
    this[this.idField] = json[this.idField]
    delete json[this.idField]
