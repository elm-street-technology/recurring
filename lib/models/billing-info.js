...
class BillingInfo extends RecurlyData {
  constructor(recurring) {
    ...
    this.__defineGetter__('href', function() {
      ...
      if (!this.account_code) {
        throw (new Error('billing info requires an account_code'))
      }

      this._href = [Account.ENDPOINT, this.account_code, 'billing_info', this.id].join('/')
      return this._href
    })
  }
  ...
  static get ENDPOINT() {
    throw new Error('Billing Info does not have a static endpoint')
  }
