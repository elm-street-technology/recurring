...
class Account extends RecurlyData {
  ...
  fetchAdjustments(callback) {
    const uri = this._resources.adjustments || (`${this.href}/adjustments`)
    this.fetchAll('Adjustment', uri, (err, results) => {
      if (err) {
        return callback(err)
      }
      this.adjustments = results
      callback(null, this.adjustments)
    })
  }

  fetchTransactions(filter, callback) {
    ...
    this.fetchAll('Transaction', uri, filter, (err, results) => {
      if (err) {
        return callback(err)
      }
  ...
  fetchSubscriptions(filter, callback) {
    if (typeof callback === 'undefined' && typeof filter === 'function') {
      callback = filter
      filter = {}
    }
    ...
    this.fetchAll('Subscription', uri, filter, (err, results) => {
      if (err) {
        return callback(err)
      }
  ...
  fetchInvoices(filter, callback) {
    ...
    this.fetchAll('Invoice', uri, filter, (err, results) => {
      if (err) {
        return callback(err)
      }
  ...
  fetchShippingAddresses(filter, callback) {
    if (typeof callback === 'undefined' && typeof filter === 'function') {
      callback = filter
      filter = {}
    }
    filter = filter || {}
    const uri = this._resources.shipping_addresses || (`${this.href}/shipping_addresses`)
    this.fetchAll('ShippingAddress', uri, filter, (err, results) => {
      if (err) {
        return callback(err)
      }
      this.shipping_addresses = results
      callback(null, this.shipping_addresses)
    })
  }
}
